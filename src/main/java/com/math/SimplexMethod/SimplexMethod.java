...
//迭代的计算直到满足条件
        while(!flag){
            ...
            Table temp = new Table(Max.get(resultCol),resultCol+1,oldTables.get(resultRow).getB()/fatherNum,rowsTemps,null);
            Tables.add(temp);
            //转换剩余的
            for(int j=0;j<EQUALITY;j++){
                if(j==resultRow){
                    ...
                    Table otherTemp = new Table(Max.get(tempXb-1),tempXb,oldTables.get(j).getB()-motherNum*fatherRow.getB(),rowsTemps,null);
...
//            System.out.println("Tables"+Tables.size());
    ...
    public void showRows(){
        ...
        for(int i=0;i<MAXPARAMS;i++){
            if(Max.get(i)!=0)MaxRows+=Max.get(i)+" X"+(i+1)+" + ";
        }
