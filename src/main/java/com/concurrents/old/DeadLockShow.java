...
class DeadLock {
    ...
    public synchronized void preparRun(Food food, DeadLock lock){
        System.out.println("准备 currentId: "+id+" resource: "+food.getName()+" prepar other: "+lock.getId());
        try {
            Thread.sleep(120);
    ...
    public synchronized void confirmRun(Food food, DeadLock lock){
        System.out.println("确认 currentId: "+id + " resource: "+food.getName()+"confirm other: "+lock.getId());
    }
}
