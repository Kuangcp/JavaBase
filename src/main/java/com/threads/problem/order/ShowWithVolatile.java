package com.threads.problem.order;
 ...
 * 但试想，如果多个线程同时并发更新这个变量，其结果也是显而易见的，最后一次的更新会覆盖前面所有更新，
 * 导致线程不安全。在方法二中，一次只有一个线程满足准入条件，因此不存在对变量的并发更新。
 * volatile的值是最新的与线程安全完全是不相干的，所以不要误用volatile实现并发控制。
